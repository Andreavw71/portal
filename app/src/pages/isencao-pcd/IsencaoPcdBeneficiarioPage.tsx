...
export function IsencaoPcdBeneficiarioPage() {
  ...
  const { etapas, marcarEtapaCompleta, obterEtapa } =
    useGerenciadorEtapas(tipoPcD);

  const { updateFormData } = useIsencaoPcd();

  const [formData, setFormData] = useState({
    nome: "João Silva da Conceição",
    cpf: "123.456.789-00",
  ...
  useEffect(() => {
    const etapaSalva = obterEtapa("beneficiario");
    if (etapaSalva?.dados) {
  ...
  const handleSubmit = () => {
    ...
    marcarEtapaCompleta("beneficiario", formData);
    updateFormData("beneficiario", formData);
                ...
                <div className="flex items-start justify-between mb-6 pb-6 border-b border-[#E5E7EB]">
                  <div>
                    ...
                    <p className="text-base text-[#4A5565]">
                      Confira as informações da pessoa com deficiência
                      (beneficiário) e complete o telefone e e-mail.
                    </p>
                  </div>
                </div>
